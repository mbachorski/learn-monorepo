...
import com.example.forage.ui.viewmodel.ForgeableViewModelFactory
...
class AddForageableFragment : Fragment() {
    ...
    private val viewModel: ForageableViewModel by activityViewModels {
        ForgeableViewModelFactory((activity?.application as BaseApplication).database.forageableDao())
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (id > 0) {
            viewModel.getItem(id).observe(viewLifecycleOwner) {
                forageable = it as Forageable
            }
